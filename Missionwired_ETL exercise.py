...
# Replace NaN values for source with empty string
pd.options.mode.chained_assignment = None
filtered_cons_df['source'].fillna(value='', inplace=True)
...
email_selected_columns = ['cons_email_id','cons_id', 'is_primary', 'email']
...
# Rename the columns
merged_email_subs_df = merged_email_subs_df.rename(columns={'create_dt':'created_dt', 'modified_dt': 'updated_dt', 'isunsub':'is_unsub', 'source':'code'})
selected_columns = ['email', 'code','is_unsub', 'created_dt', 'updated_dt']
final_df = merged_email_subs_df.loc[:, selected_columns]

# Revise data types
final_df = final_df.astype({'email': 'string', 'is_unsub': 'boolean', 'code':'string'})
final_df['created_dt'] = pd.to_datetime(final_df['created_dt'])
